...
class SAdvancedTableRow : public ITableRow, public SBorder
{
...
public:
	...
	SLATE_BEGIN_ARGS(SAdvancedTableRow< ItemType >)
	...
	{}
		...
		SLATE_ARGUMENT(const struct FExpandedArrowStyle*, ExpanderStyleSet)
		...
		SLATE_DEFAULT_SLOT(typename SAdvancedTableRow<ItemType>::FArguments, Content)
		 ...
		 */
		void Construct(const typename SAdvancedTableRow<ItemType>::FArguments& InArgs, const TSharedRef<STableViewBase>& InOwnerTableView)
	{
		/** Note: Please initialize any state in ConstructInternal, not here. This is because SAdvancedTableRow derivatives call ConstructInternal directly to avoid constructing children. **/
		...
		bool InExpanderVisibility, const TSharedRef<SWidget>& InContent)
	{
		...
		if (InExpanderVisibility)
		{
			this->ChildSlot
				...
				[
					SNew(SExtendedTreeExpander, SharedThis(this))
	...
	SAdvancedTableRow()
		: IndexInList(0)
		  , bShowSelection(true), Style(nullptr), ExpanderStyleSet(nullptr), InnerContentSlot(nullptr),
		  bChangedSelectionOnMouseDown(false),
		  bProcessingSelectionTouch(false)
	{
	}
...
protected:
	...
	void ConstructInternal(FArguments const& InArgs, TSharedRef<STableViewBase> const& InOwnerTableView)
	{
		...
		SetBorderImage(TAttribute<const FSlateBrush*>(this, &SAdvancedTableRow::GetBorder));
		this->SetForegroundColor(TAttribute<FSlateColor>(this, &SAdvancedTableRow::GetForegroundBasedOnSelection));
...
protected:
	...
	const struct FExpandedArrowStyle* ExpanderStyleSet;

	/** @see SAdvancedTableRow's OnCanAcceptDrop event */
	FOnCanAcceptDrop OnCanAcceptDrop;

	/** @see SAdvancedTableRow's OnAcceptDrop event */
