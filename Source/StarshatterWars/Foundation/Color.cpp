...
{
    int  r = (int)Red() + (int)c.Red();     if (r > 255) r = 255;
    int  g = (int)Green() + (int)c.Green();   if (g > 255) g = 255;
    int  b = (int)Blue() + (int)c.Blue();    if (b > 255) b = 255;

    Set((BYTE)r, (BYTE)g, (BYTE)b, (BYTE)Alpha());
    return *this;
}
...
{
    // d is interpreted as legacy packed ARGB: A<<24 | R<<16 | G<<8 | B
    const int add_r = (int)((d & RMask) >> RShift);
    const int add_g = (int)((d & GMask) >> GShift);
    const int add_b = (int)((d & BMask) >> BShift);

    int r = (int)Red() + add_r; if (r > 255) r = 255;
    int g = (int)Green() + add_g; if (g > 255) g = 255;
    int b = (int)Blue() + add_b; if (b > 255) b = 255;
...
{
    const float src_alpha = c.fAlpha();
    const float dst_alpha = 1.0f - src_alpha;

    const BYTE  r = (BYTE)((fRed() * dst_alpha + c.fRed() * src_alpha) * 255.0f);
    const BYTE  g = (BYTE)((fGreen() * dst_alpha + c.fGreen() * src_alpha) * 255.0f);
    const BYTE  b = (BYTE)((fBlue() * dst_alpha + c.fBlue() * src_alpha) * 255.0f);

    // preserve destination alpha (matches old behavior where alpha wasn't blended here)
    return Color(r, g, b, (BYTE)Alpha());
}
...
Color::operator*(const Color& c) const
{
    const BYTE  r = (BYTE)((fRed() * c.fRed()) * 255.0f);
    const BYTE  g = (BYTE)((fGreen() * c.fGreen()) * 255.0f);
    const BYTE  b = (BYTE)((fBlue() * c.fBlue()) * 255.0f);
...
{
    int r = fast_f2i((double)Red() * scale); if (r > 255) r = 255; if (r < 0) r = 0;
    int g = fast_f2i((double)Green() * scale); if (g > 255) g = 255; if (g < 0) g = 0;
    int b = fast_f2i((double)Blue() * scale); if (b > 255) b = 255; if (b < 0) b = 0;
    int a = fast_f2i((double)Alpha() * scale); if (a > 255) a = 255; if (a < 0) a = 0;
...
{
    int r = fast_f2i((double)Red() * scale); if (r < 0) r = 0; if (r > 255) r = 255;
    int g = fast_f2i((double)Green() * scale); if (g < 0) g = 0; if (g > 255) g = 255;
    int b = fast_f2i((double)Blue() * scale); if (b < 0) b = 0; if (b > 255) b = 255;
...
Color::Formatted() const
{
    ...
    // Fade is applied in linear 0..255 space, then packed into target format:
    if (fade != 1.0) {
        const double step = (1.0 - fade);
        ...
        DWORD a = ((DWORD)Alpha()) >> format.adown;

        return (r << format.rshift) | (g << format.gshift) | (b << format.bshift) | (a << format.ashift);
    }

    if (standard_format) {
        // Legacy "standard_format" in the old code returned the packed DWORD 'rgba'.
        // We now store as FColor, so return the legacy packed layout via Value().
        return Value();
    }

    DWORD r = ((DWORD)Red()) >> format.rdown;
    DWORD g = ((DWORD)Green()) >> format.gdown;
    DWORD b = ((DWORD)Blue()) >> format.bdown;
    DWORD a = ((DWORD)Alpha()) >> format.adown;
...
Color::TextureFormat(int keep_alpha) const
{
    ...
    // Old code used "rgba == 0" as a fast transparent test. We now store FColor.
...
{
    BYTE r = (BYTE)((c1.fRed() + (c2.fRed() - c1.fRed()) * scale) * 255.0);
    BYTE g = (BYTE)((c1.fGreen() + (c2.fGreen() - c1.fGreen()) * scale) * 255.0);
    BYTE b = (BYTE)((c1.fBlue() + (c2.fBlue() - c1.fBlue()) * scale) * 255.0);
    BYTE a = (BYTE)((c1.fAlpha() + (c2.fAlpha() - c1.fAlpha()) * scale) * 255.0);
...
Color::UseTextureFormat(const ColorFormat& cf, int alpha_level)
{
    ...
    if (cf.pal) {
        ...
        for (int i = 0; i < 256; i++) {
            const int old_texture_alpha_level = texture_alpha_level;
...
Color::SetFade(double f, Color c, int build_shade)
{
    ...
    if (format.pal && video) {
        ...
        for (int i = 0; i < 256; i++) {
            ...
            ColorIndex    cidx = ColorIndex((BYTE)i);

            entry.peRed = ((int)((cidx.fRed() - (cidx.fRed() - fade_color.fRed()) * step) * 255.0));
            entry.peGreen = ((int)((cidx.fGreen() - (cidx.fGreen() - fade_color.fGreen()) * step) * 255.0));
            entry.peBlue = ((int)((cidx.fBlue() - (cidx.fBlue() - fade_color.fBlue()) * step) * 255.0));
            entry.peFlags = 0;
        }

        // NOTE: original Starshatter likely applied this palette to the video device.
        // That call remains intentionally omitted/commented since your UE pipeline differs.
        (void)fade_palette;
    }
...
Color::BuildShadeTable()
{
    for (int shade = 0; shade < SHADE_LEVELS * 2; shade++)
        for (int index = 0; index < 256; index++)
            ColorIndex::shade_table[shade * 256 + index] = Color((BYTE)index).Shaded(shade);
}
...
Color::BuildBlendTable()
{
    for (int src = 0; src < 256; src++) {
        for (int dst = 0; dst < 256; dst++) {
            ColorIndex src_clr = ColorIndex((BYTE)src);
            ColorIndex dst_clr = ColorIndex((BYTE)dst);

            int r = (int)src_clr.Red() + (int)dst_clr.Red();
            int g = (int)src_clr.Green() + (int)dst_clr.Green();
            int b = (int)src_clr.Blue() + (int)dst_clr.Blue();
