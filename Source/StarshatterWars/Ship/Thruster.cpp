...
// Minimal Unreal include for FVector conversion requirement:
#include "Math/Vector.h"

static Sound* thruster_resource = nullptr;
static Sound* thruster_sound = nullptr;

extern UTexture2D* drive_flare_bitmap[8];
extern UTexture2D* drive_trail_bitmap[8];
	...
	avail_z(1.0f)
{
	...
	FMemory::Memzero(burn, sizeof(burn));
...
Thruster::SetShip(Ship* s)
{
	...
	if (ship) {
		...
		const double agility = design->agility;
...
Thruster::ExecTrans(double x, double y, double z)
{
		...
		ship->AltitudeAGL() > ship->Radius() / 2)
	{
		...
		double r_thrust = 0, p_thrust = 0, y_thrust = 0;
		ship->GetAngularThrust(r_thrust, p_thrust, y_thrust);

		// Roll seems to have the opposite sign from
		// the pitch and yaw thrust factors. Not sure why.

		if (r_thrust > 0)        IncBurn(ROLL_L, ROLL_R);
		else if (r_thrust < 0)   IncBurn(ROLL_R, ROLL_L);
		else                     DecBurn(ROLL_R, ROLL_L);

		if (y_thrust < 0)        IncBurn(YAW_L, YAW_R);
		else if (y_thrust > 0)   IncBurn(YAW_R, YAW_L);
		else                     DecBurn(YAW_R, YAW_L);

		if (p_thrust < 0)        IncBurn(PITCH_D, PITCH_U);
		else if (p_thrust > 0)   IncBurn(PITCH_U, PITCH_D);
	...
	if (ship && ship == Sim::GetSim()->GetPlayerShip() && ports.size() > 0) {
		...
		if (!Game::Paused() && cam_mgr && cam_mgr->GetCamera()) {
			if (!thruster_sound) {
				if (thruster_resource)
			...
			if (thruster_sound) {
				if (sound_on) {
					Point cam_loc = cam_mgr->GetCamera()->Pos();
...
void
Thruster::AddPort(int type, const FVector& loc, uint32 fire, float flare_scale)
{
	if (flare_scale == 0)
		flare_scale = scale;
...
void
Thruster::CreatePort(int type, const FVector& loc, uint32 fire, float flare_scale)
{
	UTexture2D* flare_tex = drive_flare_bitmap[subtype];
	UTexture2D* trail_tex = drive_trail_bitmap[subtype];

	if (subtype != Drive::STEALTH) {
		Sprite* flare_rep = new Sprite(flare_tex);
		flare_rep->Scale(flare_scale * 0.667f);
		flare_rep->SetShade(0);

		Bolt* trail_rep = new Bolt(flare_scale * 30, flare_scale * 8, trail_tex, true);
