    ...
    tgt_point_defense(false)
{
    ...
    if (ship && ship->Design()->auto_roll < 0) {
        FVector Torque(float(rand() - 16000), float(rand() - 16000), float(rand() - 16000));
        Torque.Normalize();
    ...
    uint32 sub_select_time = Game::GameTime() + FMath::RandRange(0u, 2000u);
...
StarshipAI::ThrottleControl()
{
    ...
    if (target || s_threat) {  // target pursuit, or retreat
        ...
        if (speed == 0) {
            double d = (ship->Location() 'TODO') // NOTE: replaced below
                ;
        }
    ...
    while (++g_iter) {
        ...
        if (g->GetDesign() && g->GetDesign()->turret_model) {
            ...
            while (++w_iter) {
                ...
                if (w->GetTurret()) {
                    FVector Tloc = w->GetTurret()->Location();
                    FVector Delta = Tloc - ship->Location();
