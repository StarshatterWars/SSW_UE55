...
void UTacRefDlg::PopulateShipDropdown()
{
    ...
    UE_LOG(LogTacRefDlg, Log, TEXT("[TacRefDlg] PopulateShipDropdown: %d ships"), ShipRowNames.Num());
}
...
void UTacRefDlg::HandleShipComboChanged(FString SelectedItem, ESelectInfo::Type SelectionType)
{
    ...
    // ComboBoxString doesn't directly expose "current index" reliably,
    // so we resolve by searching options:
