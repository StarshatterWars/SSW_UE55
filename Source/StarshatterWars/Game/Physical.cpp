...
int      UPhysical::id_key = 1;
double   UPhysical::sub_frame = 1.0 / 60.0;
...
UPhysical::UPhysical(const char* n, int t) {
	id = id_key++;
	obj_type = t;
	rep = 0;
	light = 0;
...
/*void
UPhysical::ArcadeFrame(double s)
{
	// if this object is under direction,
	// but doesn't need subframe accuracy,
	// update the control parameters:
...
void
UPhysical::AngularFrame(double seconds)
{
	if (!straight) {
...
void
UPhysical::LinearFrame(double seconds)
{
...
UPhysical::CalcFlightPath()
{
	...
	UCamera* yaw_cam;
	yaw_cam = NewObject<UCamera>();

	yaw_cam->Initialize(cam->Pos().x, cam->Pos().y, cam->Pos().z);
	yaw_cam->Clone(cam);
...
void
UPhysical::MoveTo(const Point& new_loc)
{
	//cam.MoveTo(new_loc);
}

void
UPhysical::TranslateBy(const Point& ref)
{
...
void
UPhysical::ApplyForce(const Point& force)
{
	velocity += force / mass;
}

void
UPhysical::ApplyTorque(const Point& torque)
{
...
void
UPhysical::SetThrust(double t)
{
	thrust = (float)t;
}

void
UPhysical::SetTransX(double t)
{
	trans_x = (float)t;
}

void
UPhysical::SetTransY(double t)
{
	trans_y = (float)t;
}

void
UPhysical::SetTransZ(double t)
{
	trans_z = (float)t;
}
...
void
UPhysical::SetHeading(double r, double p, double y)
{
	roll = (float)r;
	pitch = (float)p;
	yaw = (float)y;
...
void
UPhysical::LookAt(const Point& dst)
{
	cam->LookAt(dst);
}

void
UPhysical::CloneCam(const UCamera* c)
{
	cam->Clone(c);
}

void
UPhysical::SetAbsoluteOrientation(double r, double p, double y)
{
	roll = (float)r;
	pitch = (float)p;
	yaw = (float)y;
	...
	UCamera* work;
	work = NewObject<UCamera>();

	work->Initialize(Location().x, Location().y, Location().z);
	work->Aim(r, p, y);
	cam->Clone(work);
}

void
UPhysical::ApplyRoll(double r)
{
	if (r > 1)       r = 1;
	else if (r < -1) r = -1;
...
void
UPhysical::SetAngularRates(double  r, double  p, double  y)
{
	roll_rate = (float)r;
	pitch_rate = (float)p;
	yaw_rate = (float)y;
...
void
UPhysical::GetAngularRates(double& r, double& p, double& y)
{
	r = roll_rate;
	p = pitch_rate;
	y = yaw_rate;
...
void
UPhysical::SetAngularDrag(double  r, double  p, double  y)
{
	dr_drg = (float)r;
	dp_drg = (float)p;
	dy_drg = (float)y;
...
void
UPhysical::GetAngularDrag(double& r, double& p, double& y)
{
	r = dr_drg;
	p = dp_drg;
	y = dy_drg;
...
void
UPhysical::GetAngularThrust(double& r, double& p, double& y)
{
	r = 0;
	p = 0;
	y = 0;
...
void
UPhysical::SetPrimary(const Point& l, double m)
{
	primary_loc = l;
	primary_mass = m;
}
...
void
UPhysical::SetBaseDensity(double d)
{
	if (d >= 0)
		Do = (float)d;
}
...
void
UPhysical::InflictDamage(double damage, int type)
{
...
int
UPhysical::CollidesWith(UPhysical& o)
{
...
void
UPhysical::ElasticCollision(UPhysical& a, UPhysical& b)
{
...
void
UPhysical::InelasticCollision(UPhysical& a, UPhysical& b)
{
	double mass_sum = a.mass + b.mass;
...
void
UPhysical::SemiElasticCollision(UPhysical& a, UPhysical& b)
{
	double mass_sum = a.mass + b.mass;
	double mass_delta = a.mass - b.mass;
