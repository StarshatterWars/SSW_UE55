	...
	for (int i = 0; i < val->elements()->size(); i++) {
		...
		if (pdef) {
			...
			else if (pdef->name()->value() == "type")
			{
				...
				const bool bOk = RegionTypeFromString(UTF8_TO_TCHAR(ParentType), RegionType);
				if (!bOk)
				{
					//UE_LOG(LogTemp, Warning, TEXT("Unknown region type '%s' in %s"), UTF8_TO_TCHAR(ParentType), *fn);
					RegionType = EOrbitalType::NOTHING;
				}

				NewRegionData.Type = RegionType;
			}
...
bool RegionTypeFromString(const FString& InString, EOrbitalType& OutValue)
{
	...
	// 1) Try match by enum value name first (fast path)
	//    e.g. "JumpGate", "Planet"
	const int64 NameValue = Enum->GetValueByNameString(InString);
	if (NameValue != INDEX_NONE)
	{
		OutValue = static_cast<EOrbitalType>(NameValue);
		return true;
	}

	// 2) Match by DisplayName (UMETA(DisplayName="..."))
	const int32 Num = Enum->NumEnums();

	// Optional: skip the autogenerated _MAX entry if you have one
	for (int32 i = 0; i < Num; ++i)
	{
		const FString Display = Enum->GetDisplayNameTextByIndex(i).ToString();
		if (Display.Equals(InString, ESearchCase::IgnoreCase))
		{
