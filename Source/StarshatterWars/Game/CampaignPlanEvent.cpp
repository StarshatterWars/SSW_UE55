...
UCampaignPlanEvent::UCampaignPlanEvent()
{
}
...
bool
UCampaignPlanEvent::ExecScriptedEvents()
{
...
void
UCampaignPlanEvent::ProsecuteKills(CombatAction* action)
{
		...
		if (g) {
			...
			while (++iter) {
				...
				if (asset) {
					...
					ListIter<Combatant> citer = campaign->GetCombatants();
					while (++citer) {
						Combatant* c = citer.value();
...
bool
UCampaignPlanEvent::ExecStatisticalEvents()
{
	bool result = false;
...
bool
UCampaignPlanEvent::CreateEvent(CombatAssignment* a)
{
	CombatEvent* event = 0;

	if (campaign && a && a->GetResource() && RandomChance(1, 2)) {
		event_time = (int)RandomDouble(event_time, campaign->GetTime());
...
CombatAssignment*
UCampaignPlanEvent::ChooseAssignment(CombatGroup* g)
{
	...
	if (alist.size() > 0) {
		while (tries-- > 0) {
			int index = (int)RandomDouble(0, alist.size());
...
CombatEvent*
UCampaignPlanEvent::CreateEventDefend(CombatAssignment* a)
{
...
CombatEvent*
UCampaignPlanEvent::CreateEventFighterAssault(CombatAssignment* a)
{
	CombatEvent* event = 0;
	CombatGroup* group = a->GetResource();
	CombatGroup* obj = a->GetObjective();
...
CombatEvent*
UCampaignPlanEvent::CreateEventFighterStrike(CombatAssignment* a)
{
	CombatEvent* event = 0;
	CombatGroup* group = a->GetResource();
	CombatGroup* obj = a->GetObjective();
...
CombatEvent*
UCampaignPlanEvent::CreateEventFighterSweep(CombatAssignment* a)
{
	CombatEvent* event = 0;
	CombatGroup* group = a->GetResource();
	CombatGroup* obj = a->GetObjective();
...
CombatEvent*
UCampaignPlanEvent::CreateEventStarship(CombatAssignment* a)
{
	CombatEvent* event = 0;
	CombatGroup* group = a->GetResource();
	CombatGroup* obj = a->GetObjective();
...
bool
UCampaignPlanEvent::IsFriendlyAssignment(CombatAssignment* a)
{
...
bool
UCampaignPlanEvent::Success(CombatAssignment* a)
{
	if (!campaign || !a || !a->GetResource())
		return false;
...
Text
UCampaignPlanEvent::GetTeamName(CombatGroup* g)
{
	while (g->GetParent())
		g = g->GetParent();
