...
CampaignMissionFighter::CampaignMissionFighter(ACampaign* c)
{
	...
	if (!campaign || !campaign->GetPlayerGroup()) {
		UE_LOG(LogTemp, Log, TEXT("Error getting Player Group"));
		return;
	}
...
CampaignMissionFighter::IsGroundObjective(CombatGroup* obj)
{
	...
	if (obj) {
		...
		if (pgroup) {
			...
			if (zone) {
				AStarSystem* system = campaign->GetSystem(zone->System());

				if (system) {
					AOrbitalRegion* region = system->FindRegion(obj->GetRegion());

					if (region && region->Type() == AOrbitalRegion::TERRAIN) {
						ground = true;
					}
				}
			}
...
CampaignMissionFighter::SelectRegion()
{
	...
	if (zone) {
		...
		if (zone->GetRegions().size() > 1) {
			...
			AStarSystem* system = mission->GetStarSystem();
			AOrbitalRegion* rgn = 0;

			if (system)
				rgn = system->FindRegion(air_region);

			if (!rgn || rgn->Type() != AOrbitalRegion::TERRAIN)
				air_region = "";
		}
		...
		AStarSystem* s = campaign->GetSystemList()[0];
...
CampaignMissionFighter::FindCarrier(CombatGroup* g)
{
	...
	if (carrier && carrier->GetUnits().size()) {
		MissionElement* carrier_elemt = mission->FindElement(carrier->Name());
...
CampaignMissionFighter::CreatePatrols()
{
	...
	while (++iter) {
		...
		CombatGroup* squad = squad_elem->GetCombatGroup();
		CombatUnit* unit = squad_elem->GetCombatUnit();

		if (!squad_elem->IsSquadron() || !squad || !unit || unit->LiveCount() < 4)
			continue;

		if (squad->Type() == CombatGroup::INTERCEPT_SQUADRON ||
			squad->Type() == CombatGroup::FIGHTER_SQUADRON) {

			AStarSystem* system = mission->GetStarSystem();
			CombatGroup* base = squad->FindCarrier();

			if (!base)
				continue;

			AOrbitalRegion* region = system->FindRegion(base->GetRegion());
			...
			if (region->Type() == AOrbitalRegion::TERRAIN) {
			...
			if (elem) {
				elem->SetIntelLevel(INTEL_TYPE::KNOWN);
...
CampaignMissionFighter::CreateWardFreight()
{
	...
	AStarSystem* system = mission->GetStarSystem();
	AOrbitalRegion* rgn1 = system->FindRegion(elem->Region());
...
CampaignMissionFighter::CreateTargetsPatrol()
{
	...
	if (airborne) {
		...
		region = air_region;
		patrol_loc = base_loc +
			RandomDirection() * RandomDouble(60e3, 100e3);
	}
	else {
		patrol_loc = base_loc +
			RandomDirection() * RandomDouble(110e3, 160e3);
	}
	...
	int ntargets = (int)RandomDouble(2.0, 5.1);
...
CampaignMissionFighter::CreateTargetsSweep()
{
	...
	double traverse = PI;
	double a = RandomDouble(-PI / 2, PI / 2);
	Point  base_loc = player_elem->Location();
	...
	while (traverse > 0) {
		double a1 = RandomDouble(PI / 4, PI / 2);
		traverse -= a1;
		a += a1;
...
CampaignMissionFighter::CreateTargetsIntercept()
{
	...
	while (ninbound--) {
		...
		if (elem) {
			...
			if (e2) {
				e2->SetIntelLevel(INTEL_TYPE::KNOWN);
...
CampaignMissionFighter::PlanetaryInsertion(MissionElement* elem)
{
	...
	AStarSystem* system = mission->GetStarSystem();
	AOrbitalRegion* rgn1 = system->FindRegion(elem->Region());
	AOrbitalRegion* rgn2 = system->FindRegion(air_region);
	Point          npt_loc = elem->Location();
	Instruction* n = 0;
	PlayerData* p = PlayerData::GetCurrentPlayer();
