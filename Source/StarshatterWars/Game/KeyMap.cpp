...
static KeyName key_action_table[] = {
	...
	{ KeyMap::KEY_FLIGHT,  KEY_CONTROL_MODEL,   "KEY_CONTROL_MODEL",  nullptr                },

	{ KeyMap::KEY_FLIGHT,  KEY_MOUSE_SELECT,    "KEY_MOUSE_SELECT",   nullptr                },
	{ KeyMap::KEY_FLIGHT,  KEY_MOUSE_SENSE,     "KEY_MOUSE_SENSE",    nullptr                },
	{ KeyMap::KEY_FLIGHT,  KEY_MOUSE_SWAP,      "KEY_MOUSE_SWAP",     nullptr                },
	{ KeyMap::KEY_FLIGHT,  KEY_MOUSE_INVERT,    "KEY_MOUSE_INVERT",   nullptr                },
	{ KeyMap::KEY_FLIGHT,  KEY_MOUSE_ACTIVE,    "KEY_MOUSE_ACTIVE",  "Toggle Mouse Ctrl"     },

	{ KeyMap::KEY_FLIGHT,  KEY_JOY_SELECT,      "KEY_JOY_SELECT",     nullptr                },
	{ KeyMap::KEY_FLIGHT,  KEY_JOY_RUDDER,      "KEY_JOY_RUDDER",     nullptr                },
	{ KeyMap::KEY_FLIGHT,  KEY_JOY_THROTTLE,    "KEY_JOY_THROTTLE",   nullptr                },
	{ KeyMap::KEY_FLIGHT,  KEY_JOY_SENSE,       "KEY_JOY_SENSE",      nullptr                },
	{ KeyMap::KEY_FLIGHT,  KEY_JOY_DEAD_ZONE,   "KEY_JOY_DEAD_ZONE",  nullptr                },
	{ KeyMap::KEY_FLIGHT,  KEY_JOY_SWAP,        "KEY_JOY_SWAP",       nullptr                },

	{ KeyMap::KEY_FLIGHT,  KEY_AXIS_YAW,               "KEY_AXIS_YAW",             nullptr },
	{ KeyMap::KEY_FLIGHT,  KEY_AXIS_PITCH,             "KEY_AXIS_PITCH",           nullptr },
	{ KeyMap::KEY_FLIGHT,  KEY_AXIS_ROLL,              "KEY_AXIS_ROLL",            nullptr },
	{ KeyMap::KEY_FLIGHT,  KEY_AXIS_THROTTLE,          "KEY_AXIS_THROTTLE",        nullptr },
	{ KeyMap::KEY_FLIGHT,  KEY_AXIS_YAW_INVERT,        "KEY_AXIS_YAW_INVERT",      nullptr },
	{ KeyMap::KEY_FLIGHT,  KEY_AXIS_PITCH_INVERT,      "KEY_AXIS_PITCH_INVERT",    nullptr },
	{ KeyMap::KEY_FLIGHT,  KEY_AXIS_ROLL_INVERT,       "KEY_AXIS_ROLL_INVERT",     nullptr },
	{ KeyMap::KEY_FLIGHT,  KEY_AXIS_THROTTLE_INVERT,   "KEY_AXIS_THROTTLE_INVERT", nullptr },
...
int KeyMap::GetKeyAction(const char* act_str)
{
	...
	const int nactions = (int)(sizeof(key_action_table) / sizeof(KeyName));
...
int KeyMap::GetKeyKey(const char* key_str)
{
	...
	const int nkeys_local = (int)(sizeof(key_key_table) / sizeof(KeyName));

	for (int i = 0; i < nkeys_local; i++)
...
KeyMap::KeyMap()
	: nkeys(0)
{
	const int n = BuildDefaultKeyMap();
	DefaultKeyMap(n);
}
	...
	while (fgets(line, sizeof(line), f)) {
		...
		if (act != -1 && key != -1) {
			...
			for (int i = 0; i < max_keys && !mapped; i++) {
				if (map[i].act == act) {
					UE_LOG(LogTemp, Log, TEXT("Remapping: '%hs' => %hs(%d) %hs(%d) %hs(%d)"),
			...
			if (!mapped) {
				UE_LOG(LogTemp, Log, TEXT("Mapping: '%hs' => %hs(%d) %hs(%d) %hs(%d)"),
					act_str, key_str, key, alt_str, alt, joy_str, joy);
		...
		if (nkeys >= max_keys - 1) {
			UE_LOG(LogTemp, Warning, TEXT("Too many keys in configuration..."));
			break;
		}
	}
...
KeyMap::SaveKeyMap(const char* filename, int max_keys)
{
	...
	for (int i = 0; i < nkeys; i++) {
		if (map[i].act >= KEY_CONTROL_MODEL &&
			...
			const int a = GetKeyActionIndex(map[i].act);
			fprintf(f, "%-24s =%d\n", key_action_table[a].name, map[i].key);
		}

		else if (map[i] != defmap[i]) {
			const int a = GetKeyActionIndex(map[i].act);
			const int k = GetKeyKeyIndex(map[i].key);
			const int s = GetKeyKeyIndex(map[i].alt);
			const int j = GetKeyKeyIndex(map[i].joy);
...
KeyMap::DescribeKey(int vk, int shift, int j)
{
	...
	const char* joy = nullptr;

	const int nkeys_local = (int)(sizeof(key_key_table) / sizeof(KeyName));

	for (int i = 0; i < nkeys_local; i++) {
		if (vk > 0 && vk == key_key_table[i].key) {
			key = key_key_table[i].desc ? key_key_table[i].desc : key_key_table[i].name;
		}

		if (shift > 0 && shift == key_key_table[i].key) {
			alt = key_key_table[i].desc ? key_key_table[i].desc : key_key_table[i].name;
		}

		if (j > 0 && j == key_key_table[i].key) {
			joy = key_key_table[i].desc ? key_key_table[i].desc : key_key_table[i].name;
		}
	}
