 ...
 */
UCLASS()
class STARSHATTERWARS_API UPhysical : public UObject
{
	GENERATED_BODY()
...
public:
	...
	UPhysical();
	UPhysical(const char* n, int t = 0);
	virtual ~UPhysical();

	int operator == (const UPhysical& p) const { return id == p.id; }
	...
	virtual int       CollidesWith(UPhysical& o);
	static  void      ElasticCollision(UPhysical& a, UPhysical& b);
	static  void      InelasticCollision(UPhysical& a, UPhysical& b);
	static  void      SemiElasticCollision(UPhysical& a, UPhysical& b);
	...
	const UCamera* Cam() const { return cam; }
	...
	virtual void      CloneCam(const UCamera* cam);
