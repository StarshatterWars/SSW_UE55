...
void
UCampaignPlanMission::ExecFrame()
{
...
UCampaignPlanMission::PlanStarshipMission()
{
...
CampaignMissionRequest*
UCampaignPlanMission::PlanFighterMission()
{
	CampaignMissionRequest* request = 0;
...
CampaignMissionRequest*
UCampaignPlanMission::PlanCampaignMission()
{
	CampaignMissionRequest* request = 0;
...
CampaignMissionRequest*
UCampaignPlanMission::PlanStrategicMission()
{
	CampaignMissionRequest* request = 0;
...
CampaignMissionRequest*
UCampaignPlanMission::PlanRandomStarshipMission()
{
...
CampaignMissionRequest*
UCampaignPlanMission::PlanRandomFighterMission()
{
	CampaignMissionRequest* request = 0;
	...
	if (type >= Mission::AIR_PATROL && type <= Mission::AIR_INTERCEPT) {
		...
		if (zone && zone->GetRegions().size() > 1) {
			...
			if (system) {
				AOrbitalRegion* rgn = system->FindRegion(air_region);

				if (rgn && rgn->Type() == (int) AOrbitalRegion::TERRAIN)
					airborne = true;
			}
		}
