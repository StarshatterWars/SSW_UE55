...
bool MissionTemplate::Load(const char* fn, const char* fp)
{
	return false;
}

MissionElement* MissionTemplate::FindElement(const char* en)
{
	return nullptr;
}
...
Text MissionTemplate::MapShip(Text sn)
{
	return Text();
}

Text MissionTemplate::MapCallsign(const char* cn, int iff)
{
	return Text();
}
...
MissionTemplate::ParseCallsign(TermStruct* val)
{
	...
	if (callname.length() > 0 && iff >= 0)
		MapCallsign(callname, iff);
}
