			...
			if (def) {
				...
				if (defname == "primary" ||
					...
					if (!def->term() || !def->term()->isStruct()) {
						...
						// Texture loading:
						// Bitmap usage removed; DataLoader is expected to load UTexture2D* for these fields.
						if (design->anim_length > 0) {
							// Store the first frame in 'animation'; additional frames are intentionally ignored here
							// (the original implementation stored an array of Bitmap frames).
